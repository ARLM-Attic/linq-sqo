//
// Luke Hoban's LINQ raytracer instrumented with AsFEnumerable and AsEnumerable calls.
// http://blogs.msdn.com/lukeh/archive/2007/10/01/taking-linq-to-objects-to-extremes-a-fully-linqified-raytracer.aspx
//
// Search for /*<->*/ to find all places where conversions happen.
//
...
namespace RayTracer
{
    public class RayTracer
    {
        ...
        {
            var pixelsQuery =
                from y in Enumerable.Range(0, screenHeight).AsFEnumerable(/*<->*/)
                let recenterY = -(y - (screenHeight / 2.0)) / (2.0 * screenHeight)
                select from x in Enumerable.Range(0, screenWidth).AsFEnumerable(/*<->*/)
                       ...
                       let computeTraceRay = (Func<Func<TraceRayArgs, Color>, Func<TraceRayArgs, Color>>)
                        (f => traceRayArgs =>
                         (from isect in
                              from thing in traceRayArgs.Scene.Things.AsFEnumerable(/*<->*/)
                          ...
                          let naturalColors =
                              from light in traceRayArgs.Scene.Lights.AsFEnumerable(/*<->*/)
            ...
            foreach (var row in pixelsQuery.AsEnumerable(/*<->*/))
                foreach (var pixel in row.AsEnumerable(/*<->*/))
